...
public:
	...
	static void copy(const SparseMatrix<Element>& A, SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& B);

	template <typename Element, typename Index>
	static void copy(SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, SparseMatrix<Element>& B);

	template <typename Element, typename Index>
	static void transformSparseToHybridRows(SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A);

	template <typename Element, typename Index>
	static void transformHybridToSparseRows(SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A);
	...
	template <typename Element, typename Index>
	static void dumpMatrixAsPbmImage(SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, const char *outputFileName);
	...
	template <typename Element, typename Index>
	static bool equal(const Modular<Element>& R, SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, const SparseMatrix<Element>& B);

	template <typename Element, typename Index>
	static bool equal(const Modular<Element>& R, const SparseMatrix<Element>& A, SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& B);
	...
	template<class Ring>
	static void loadF4Matrix__low_memory_syscall_no_checks(const Ring &R, SparseMatrix<typename Ring::Element>& A, const char *fileName);

	template <typename Element>
	static std::pair<uint64, double> getMatrixSizeAndDensity(const SparseMatrix<Element>& A);

	template <typename Element, typename Index>
	static std::pair<uint64, double> getMatrixSizeAndDensity(const SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, bool exact = false);
...
private:
	...
	template <typename Element, typename Index>
	static void sparse_to_bloc_copyTopDowLeftRight(const SparseMatrix<Element>& A, SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& B);

	template <typename Element, typename Index>
	static void sparse_to_bloc_copyDownTopLeftRight(const SparseMatrix<Element>& A, SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& B);



	template <typename Element, typename Index>
	static void bloc_to_sparse_copyDownTopRightLeft(const SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, SparseMatrix<Element>& B);

	template <typename Element, typename Index>
	static void bloc_to_sparse_copyTopDowLeftRight(const SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, SparseMatrix<Element>& B);

	template <typename Element, typename Index>
	static void bloc_to_sparse_copyDownTopLeftRight(const SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& A, SparseMatrix<Element>& B);
