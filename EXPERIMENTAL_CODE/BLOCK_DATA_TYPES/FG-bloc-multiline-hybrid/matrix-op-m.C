...
static void copyDenseArraysToMultilineVector64(const Ring& R,
		...
		bool reduce)
{
	MultiLineVector<typename Ring::Element> tmp(v.nb_lines());
...
static inline long head(const MultiLineVector<uint16>& v, const uint16 line_idx, uint16& a, uint32& head_idx)
{
	uint16 val=0;
	for(uint32 i=0; i<v.size(); ++i)
	{
...
template <typename Ring>
static inline void normalize_multiline(const Ring& R, MultiLineVector<uint16>& v)
{
	uint32 idx;
	uint16 h1=0, h2=0;
