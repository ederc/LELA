...
#define GET_ROW(Matrix, column, index)	\
	Matrix[index/DEFAULT_BLOC_WIDTH][column][index%DEFAULT_BLOC_HEIGHT]

template <typename Element, typename Index>
static uint32 echelonize_blocs_column(const Modular<Element>& R, 
					SparseBlocMatrix<SparseMultilineBloc<Element, Index> >& M,
					Element **Permutation,
					bool *in_row_permutation,
					uint32 column)
{
	...
	normalize_multiline(R, GET_ROW(M, column, 0));
	...
	copyMultilineToDenseArray(GET_ROW(M, column, i), tmpDenseArray1, tmpDenseArray2);
	...
	{
		copyDenseArraysToMultilineVector(R, tmpDenseArray2, tmpDenseArray1, coldim, GET_ROW(M, column, i));
		in_row_permutation[i] = true;
	}
	else
	{
		copyDenseArraysToMultilineVector(R, tmpDenseArray1, tmpDenseArray2, coldim, GET_ROW(M, column, i));
		in_row_permutation[i] = false;
	}

	cout << "write back " << endl;

	normalize_multiline(R, GET_ROW(M, column, i));

	if(!GET_ROW(M, column, i).empty ())
	{
		piv[npiv] = i;
		npiv++;
	}
