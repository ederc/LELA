...
void MatrixUtils::sparse_to_bloc_copyTopDowLeftRight(const SparseMatrix<Element>& A, SparseBlocMatrix<ContiguousBloc<Element, Index> >& B)
{
	...
	std::vector<Element> tmp_buffer_val(B.bloc_width(), 0);
	std::vector<Element> tmp_buffer_pos(B.bloc_width(), 0);
	...
	for(i_A = A.rowBegin (), i=0; i_A != A.rowEnd (); ++i_A, ++i)
	{
		...
		for(it = i_A->begin (); it != i_A->end (); ++it)
		{
			...
			{
				if((float)tmp_buffer_val.size() / B.bloc_width() >= B.get_HYBRID_REPRESENTATION_THRESHOLD ())	//dense
				{
					uint32 tmp_idx=0;
					for (uint32 t = 0; t < B.bloc_width(); ++t)
					{
						if(tmp_idx < tmp_buffer_pos.size() && tmp_buffer_pos[tmp_idx] == t)
						{
							B[row_bloc_idx][old_bloc_idx].push_back_value(tmp_buffer_val[tmp_idx]);
							++tmp_idx;
						}
						else
							B[row_bloc_idx][old_bloc_idx].push_back_value(0);
					}

					B[row_bloc_idx][old_bloc_idx].set_row_size(line_idx_in_bloc, B.bloc_width());
				}
				else	//sparse row
				{
					for(uint32 t=0; t<tmp_buffer_val.size(); ++t)
					{
						B[row_bloc_idx][old_bloc_idx].push_back_value(tmp_buffer_val[t]);
						B[row_bloc_idx][old_bloc_idx].push_back_pos(tmp_buffer_pos[t]);
					}
			...
			tmp_buffer_val.push_back(it->second);
			tmp_buffer_pos.push_back(elt_idx_in_line);
		...
		{
			//B[row_bloc_idx][old_bloc_idx].set_row_size(line_idx_in_bloc, nb_elts_added);
			if((float)tmp_buffer_val.size() / B.bloc_width() >= B.get_HYBRID_REPRESENTATION_THRESHOLD ())
			{
				uint32 tmp_idx=0;
				for (uint32 t = 0; t < B.bloc_width(); ++t)
				{
	...
	for(uint32 i=0; i<A.rowBlocDim (); ++i)			//for each row of blocs in A
	{
		...
		for(uint32 j=0;j<A[i].size (); ++j)		//for each bloc in the row
		{
			...
			for(uint16 k=0; k<A.bloc_height () && k<(B.rowdim() - curr_row_B); ++k)	//for each row in the bloc
			{
				...
				{
					check_equal_or_raise_exception(A[i][j].get_row_size(k), A.bloc_width());
					Element e;
					for (uint32 l = 0; l < A[i][j].get_row_size(k); ++l)
					{
						e = A[i][j].value_in_row(k, l);
						if (e != 0)
						{
							B[curr_row_B + k].push_back(
									typename SparseMatrix<Element>::Row::value_type(
