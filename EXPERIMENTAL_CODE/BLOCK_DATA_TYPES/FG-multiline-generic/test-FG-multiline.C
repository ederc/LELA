							...
							SparseMultilineMatrix<uint16>& D)
{
	...
	uint64 *tmpDenseArrayD[NB_LINES_PER_BLOC];
	uint64 *tmpDenseArrayC[NB_LINES_PER_BLOC];
	uint16 *Vals[NB_LINES_PER_BLOC];

	for(uint16 j=0; j<NB_LINES_PER_BLOC; ++j)
		tmpDenseArrayD[j] = new uint64[D_coldim];

	for(uint16 j=0; j<NB_LINES_PER_BLOC; ++j)
		tmpDenseArrayC[j] = new uint64[C_coldim];
	...
	for(i_C = C.rowBegin (), i_D = D.rowBegin (); i_C != C.rowEnd (); ++i_C, ++i_D){
		...
		TIMER_START_(RazArrayTimer);
			 razArray64(tmpDenseArrayD, NB_LINES_PER_BLOC, D_coldim);
			 razArray64(tmpDenseArrayC, NB_LINES_PER_BLOC, C_coldim);
		TIMER_STOP_(RazArrayTimer);

		TIMER_START_(CopySparseVectorToDenseArrayTimer);
			copyMultilineToDenseArrays64(*i_D, tmpDenseArrayD, NB_LINES_PER_BLOC);
			copyMultilineToDenseArrays64(*i_C, tmpDenseArrayC, NB_LINES_PER_BLOC);
		TIMER_STOP_(CopySparseVectorToDenseArrayTimer);

		typename Ring::Element Cv_col1[NB_LINES_PER_BLOC];
		uint32 Cp1;
		uint32 Cp[NB_LINES_PER_BLOC];
		int nb_consecutive = 0;
		...
		for(uint32 j=i_C->IndexData[0]; j<C_coldim; ++j)
		{
			...
			for(uint16 k=0; k<NB_LINES_PER_BLOC; ++k)
			{
				R.copy(Cv_col1[k], tmpDenseArrayC[k][j] % R._modulus);
			...
			if(Cp1 % NB_LINES_PER_BLOC == 0 && j < C_coldim - NB_LINES_PER_BLOC && C_coldim > NB_LINES_PER_BLOC)
			{
				for(uint16 l=1; l<NB_LINES_PER_BLOC; ++l)
				{
					Cp[l] = j+l; //i_A->IndexData[j+l];
					if((Cp[l] - Cp[0]) >= NB_LINES_PER_BLOC)
						break;
					...
					for(uint16 k=0; k<NB_LINES_PER_BLOC; ++k)
					{
						R.copy(Vals[k][idx], tmpDenseArrayC[k][j+l] % R._modulus);// i_A->at_unchecked(k, j+l));
				...
				if(nb_consecutive > 0)		//AXPY2
				{
					TIMER_START_(Axpy2Timer);
						axpy2(Vals,
								nb_consecutive+1,
								A[Cp1/A.nb_lines_per_bloc()],
								tmpDenseArrayC);
					TIMER_STOP_(Axpy2Timer);
		...
		TIMER_START_(CopyDenseArrayToSparseVectorTimer);
			copyDenseArraysToMultilineVector64(R, tmpDenseArrayD, NB_LINES_PER_BLOC, D_coldim, *i_D, true);
		TIMER_STOP_(CopyDenseArrayToSparseVectorTimer);
	}
#ifdef SHOW_PROGRESS
	report << "\r                                                                    \n";
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(equal_reverse(R, sub_C, sub_C_))
		cout << "sub_C EQUAL" << endl;
	else
		cout << "sub_C NOT EQUAL" << endl;

	if(equal(R, sub_D, sub_D_))
	...
	commentator.start("Reduce CD [Multiline]");
		reduceCD(R, sub_A, sub_B, sub_C, sub_D);
	commentator.stop(MSG_DONE);
