...
public:
				...
				_bloc_width (bloc_width)
	{
		...
		_row_sz = Rep (_bloc_height, 0);
		_rows_starting_offset = std::vector<uint32> (_bloc_height, 0);
	...
	ContiguousBloc(const ContiguousBloc<Element, Index>& other) :
				...
				_rows_starting_offset (other._rows_starting_offset)
	{
		//_rows_starting_offset[0] = 0;
	}
	...
	inline	void	construct_rows_starting_offset ()
	{
		uint32 acc = 0;
		_rows_starting_offset[0] = acc;

		for(uint32 i=0; i<_bloc_height; ++i)
		{
			acc += _row_sz[i];
			_rows_starting_offset[i+1] = acc;
		}
	}
	...
	{
		return _values[_rows_starting_offset[row] + i];
	}

	inline Element pos_in_row(const uint16 row, const Index i) const
	{
		return _positions[_rows_starting_offset[row] + i];
	}

	inline	uint32	get_row_end_offset	(const uint16 row) const	{ return _rows_starting_offset[row]; }
	...
	inline 	size_t	size	()	const	{ return _rows_starting_offset.size()-1; }
	inline	bool	empty	()	const	{ return _rows_starting_offset.empty(); }
	...
	{ 
		...
		_rows_starting_offset.clear ();
		_rows_starting_offset.resize (_bloc_height+1, 0);;
...
private:
	...
	std::vector<Element>		_values;				//the values of all the elements in the bloc
	std::vector<Index>		_positions;				//positions of all the elements in the bloc	in their corresponding row
	std::vector<Index>		_row_sz;
	std::vector<uint32>		_rows_starting_offset;				//holds the starting index of first element of each row in _values
};
