...
public:
	...
	void constructSubMatrices(SparseMatrix<Element>& M,
			SparseBlocMatrix<Element, Index>& A, SparseBlocMatrix<Element, Index>& B,
			SparseBlocMatrix<Element, Index>& C, SparseBlocMatrix<Element, Index>& D,
			bool destruct_original_matrix)
	{
		...
		if (!_index_maps_constructed)
		{
			...
			A = SparseBlocMatrix<Element, Index>(Npiv, Npiv,
					SparseBlocMatrix<Element, Index>::ArrangementDownTop_RightLeft,
					false, A.block_height(), A.block_width());

			B = SparseBlocMatrix<Element, Index>(Npiv, M.coldim() - Npiv,
					SparseBlocMatrix<Element, Index>::ArrangementDownTop_LeftRight,
					true, B.block_height(), B.block_width()); //true: add fill blocs since they might be used

			C = SparseBlocMatrix<Element, Index>(M.rowdim() - Npiv, Npiv,
					SparseBlocMatrix<Element, Index>::ArrangementDownTop_RightLeft,
					false, C.block_height(), C.block_width());

			D = SparseBlocMatrix<Element, Index>(M.rowdim() - Npiv, M.coldim() - Npiv,
					SparseBlocMatrix<Element, Index>::ArrangementDownTop_LeftRight,
					true, D.block_height(), D.block_width()); //true: add fill blocs since they might be used
		}
