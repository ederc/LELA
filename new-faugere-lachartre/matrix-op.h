...
public:
	...
	static void reducePivotsByPivots(Modular<uint16>& R, const Matrix& A, Matrix& B, bool t, uint8 STEP);
	...
	template <typename Matrix>
	static void reduceNonPivotsByPivots(Modular<uint16>& R, const Matrix& C, const Matrix& B, Matrix& D, bool t);
	
	template <typename Ring, typename Matrix>
	static size_t  echelonize(const Ring& R, Matrix& A);
	
	template <typename Matrix>
	static size_t  echelonize(const Modular<uint16>& R, Matrix& A, bool t);
