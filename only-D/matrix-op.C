...
template <typename Matrix>
void MatrixOp::reduceCD(Modular<uint16>& R, const Matrix& A, const Matrix& B, const Matrix& C, Matrix& D)
{
	assert(C.rowdim () == D.rowdim ());
	assert(C.coldim () == B.rowdim ());
	assert(B.coldim () == D.coldim ());
	...
	uint64 tmpDenseArrayD[D_coldim];
	uint64 tmpDenseArrayC[C_coldim];
	...
	TIMER_DECLARE_(razArrayD);
	TIMER_DECLARE_(razArrayC);
	TIMER_DECLARE_(copySparseVectorToDenseArray64C);
	TIMER_DECLARE_(copySparseVectorToDenseArray64D);
	TIMER_DECLARE_(copyDenseArrayToSparseVector64);
	TIMER_DECLARE_(AxpyC);
	TIMER_DECLARE_(AxpyD);
	TIMER_DECLARE_(AxpyTotal);
	...
	for(i_C = C.rowBegin (), i_D = D.rowBegin (); i_C != C.rowEnd (); ++i_C, ++i_D){
		...
		TIMER_START_(razArrayD);
			razArray64(tmpDenseArrayD, D_coldim);
		TIMER_STOP_(razArrayD);

		TIMER_START_(razArrayC);
			razArray64(tmpDenseArrayC, C_coldim);
		TIMER_STOP_(razArrayC);

		TIMER_START_(copySparseVectorToDenseArray64D);
			copySparseVectorToDenseArray64(i_D->begin (), i_D->end (), tmpDenseArrayD);
		TIMER_STOP_(copySparseVectorToDenseArray64D);

		TIMER_START_(copySparseVectorToDenseArray64C);
		...
		TIMER_STOP_(copyDenseArrayToSparseVector64);
	}
	...
	TIMER_REPORT_(razArrayD);
	TIMER_REPORT_(razArrayC);
	TIMER_REPORT_(copySparseVectorToDenseArray64C);
	TIMER_REPORT_(copySparseVectorToDenseArray64D);
	TIMER_REPORT_(copyDenseArrayToSparseVector64);
	TIMER_REPORT_(AxpyC);
	TIMER_REPORT_(AxpyD);
