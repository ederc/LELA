...
// 						v2_stack.push_back (it2->second);
			...
			bool destruct_original_matrix)
	{
		...
		for (int i = M.rowdim() - 1; i >= 0; --i) 	//non pivot rows
		{
			...
			if(curr_vect_in_bloc == D.bloc_height ())
			{
				write_row_blocs_to_LeftMultiline_RightBloc_matrix(M, C, D, rows_idxs_horizontal_bloc, curr_vect_in_bloc, row_bloc_idx);
			...
			bool destruct_original_matrix)
	{
		...
		for (int i = M.coldim () - 1; i >= 0 ; --i)
		{
			...
			if(curr_vect_in_bloc == B.bloc_height ())
			{
				write_row_blocs_to_LeftMultiline_RightBloc_matrix(M, A, B, rows_idxs_horizontal_bloc, curr_vect_in_bloc, row_bloc_idx);
...
//						cout << "FREE ROW D" << endl;
	...
	void combineInnerIndexer(SequentialIndexer<Element, Index>& inner_idxr, bool only_rows = false)
	{
