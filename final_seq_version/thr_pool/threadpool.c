...
threadpool create_threadpool(int num_threads_in_pool)
{
	...
	//XXX: modif
	if (pthread_cond_init(&(pool->task_finished), NULL))
	{
		fprintf(stderr, "CV initiation error!\n");
		return NULL;
	}
...
void destroy_threadpool(threadpool destroyme)
{
	...
	//XXX
	pthread_cond_destroy(&(pool->task_finished));

	return;
}


void wait_all_tasks(threadpool from_me)
{
	_threadpool *pool = (_threadpool *) from_me;

	fprintf(stderr, "[thread pool] Waiting for tasks to finish\n");

	pthread_mutex_lock(&(pool->qlock)); //get the q lock.

		while (pool->nb_tasks_running != 0)
		{
			//wait until the condition says its emtpy and give up the lock.
			pthread_mutex_unlock(&(pool->qlock)); //get the qlock.
			pthread_cond_wait(&(pool->task_finished), &(pool->qlock));
		}

	pthread_mutex_unlock(&(pool->qlock));

	fprintf(stderr, "[thread pool] All tasks done\n");
}
